...
SoftwareSerial rigCat(10,11); // rx,tx
...
void FT857D::begin(unsigned long baud) {
    rigCat.begin(baud);
}
...
// todo set the clarifier frequency
...
void FT857D::squelchFreq(unsigned int freq, char * sqlType) {
    unsigned char rigSqlFreq[5] = {0x00,0x00,0x00,0x00,0x00};
    if (strcasecmp(sqlType,"C")==0) rigSqlFreq[4] = CAT_SQL_CTCSS_SET;
    if (strcasecmp(sqlType,"D")==0) rigSqlFreq[4] = CAT_SQL_DCS_SET;

    unsigned char freq_bcd[5];
    to_bcd_be(freq_bcd, (unsigned long) freq, 4);
